...
class tripleo::profile::base::database::mysql (
  ...
  if $step >= 2 and $sync_db {
    ...
    if hiera('vault_enabled', false) {
      # NOTE: winking extension
      tripleo::profile::base::database::mysql::include_and_check_auth{'vault::db::mysql':}
    }
    if hiera('michman_enabled', false) {
      tripleo::profile::base::database::mysql::include_and_check_auth{'michman::db::mysql':}
    }
  }
